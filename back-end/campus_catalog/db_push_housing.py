...
def populate_geocode():
    results = db.session.query(
        housing.c.property_id, housing.c.address, housing.c.city, housing.c.state
    ).all()
    for location in results:
        ...
        db.session.query(housing).filter(housing.c.property_id == property_id).update(
            {housing.c.lat: lat, housing.c.lon: lon}
        )
        db.session.commit()
...
def populate_housing():
    ...
    location_list = (
        db.session.query(university.c.city, university.c.state)
        .filter(university.c.rank <= 200)
        .order_by(university.c.state)
        .distinct()
    )
...
def populate_property(property_type, client, max_page, max_property, location):
    ...
    run = client.actor("tugkan/apartments-scraper").call(
        run_input=run_input, build="latest", memory_mbytes=8192, timeout_secs=0
    )
    ...
    for property in json_response:
        ...
        if (
            property["beds"] is None
            or property["baths"] is None
            or property["sqft"] is None
        ):
            continue
        ...
        if property["propertyType"] == "apartment" and (
            property["isVerified"] == False or property["rating"] is None
        ):
            continue
...
def parse_json(property):
    ...
    if (
        db.session.query(housing)
        .filter(
            and_(
                housing.c.property_id == house.get_id(),
                housing.c.property_type == house.get_type(),
            )
        )
        .count()
        < 1
        ...
        populate_photos(
            result["property_id"], result["property_type"], property["photos"]
        )
        return 1
    return 0
...
def populate_photos(prop_id, prop_type, photos):
    for photo in photos:
        ...
        if (
            db.session.query(housingImages)
            .filter(housingImages.c.image_url == photo)
            .count()
            < 1
        ):
...
def split_range(range):
    ...
    if len(min_max) == 1:
        return (
            [0, 0]
            if min_max[0].find("Studio") != -1
            else [float(min_max[0]), float(min_max[0])]
        )
    else:
        return (
            [0, float(min_max[1])]
            if min_max[0].find("Studio") != -1
            else [float(min_max[0]), float(min_max[1])]
        )
