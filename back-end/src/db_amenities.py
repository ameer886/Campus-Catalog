...
locations = (
    db.session.query(University.city, University.state)
...
for loc in locations:
    ...
    try:
        search = yelp_api.search_query(
            term=term, location=location, limit=limit, sort_by=sort_by
        )

        # Filter results
        for item in [
            x
            for x in search["businesses"]
            if x["rating"] >= 3.5 and x["review_count"] >= 100
        ]:
            # Generate id
            ...
            hours = "NaN"
            g_place_search = gmaps.places(
                query=amen_name, location=(latitude, longitude)
            )
            if len(g_place_search["results"]) > 0:
                g_place_id = g_place_search["results"][0]["place_id"]
                g_place = gmaps.place(place_id=g_place_id)
                if g_place["result"].get("opening_hours") and g_place["result"][
                    "opening_hours"
                ].get("weekday_text"):
                    hours = "\n".join(
                        g_place["result"]["opening_hours"]["weekday_text"]
                    )
            ...
            for category in categories_query:
                ...
                categories.append(
                    AmenitiesCategories(
                        id=id, amen_id=amen_id, category=category["title"]
                    )
                )
