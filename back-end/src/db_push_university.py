...
while page_num < page_limit:
    for item in data['results']:
        ...
        if(college_rank is not None):
            place_url = 'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?fields=photos%2Cname&input=' + item['latest.school.name'] + '&locationbias=circle%3A2000%40' + str(item['location.lat']) + '%2C' + str(item['location.lon']) + ' &inputtype=textquery&key=' + os.getenv("GOOGLE_PLACE_API_KEY")
            ...
            if(p_data['status'] == 'OK'):
                try:
                    ...
                    image_url = 'https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photo_reference='+ photo_ref +'&key=' + os.getenv("GOOGLE_PLACE_API_KEY")
